...
- **LOAD MORE TOURNAMENTS (pagination)**: `/events` is paginated (~50/page, 59 pages total). `_fetch_tournaments_page(page)` fetches `/events` (p1) or `/events/?page=N`; `_get_total_pages()` parses pagination. `get_tournaments(refresh=False, pages=N)` now returns a dict `{tournaments, fetched_at, error, total_pages, pages_fetched}` and fetches ONLY the missing pages (page 1 re-fetched only on refresh), merging into `tournaments_cache.json` (`pages`/`total_pages` keys; old-format cache learns total from p1 once). Sequential with 1s pause. Partial failure → returns what it has + error; no-op load-more retry does NOT reset the 24h TTL. `/api/tournaments?pages=N` returns the new fields. UI: "Load more tournaments (page X of Y)" fetches +5 pages per click (1 → 1-5 → 1-10…; hidden when all loaded; total_pages=1 = unknown → button stays), status shows "X loaded (page A of B)", refresh icon spins; refresh + load-more buttons restyled as accent-red pills.
