...
**Implementation:**
- Toolbar: Brush, Eraser, Shape Picker (Line, Rectangle, Filled Rectangle, Highlighter, Rounded Rectangle, Curved Arrow, Smilies, Text, etc. -- Text tool opens a canvas-positioned horizontal resizable `contenteditable` rich-text editor with a drag-to-move floating styles toolbar supporting selection-level formatting: Bold, Italic, Underline, size increase/decrease A-/A+ buttons, line gap L-/L+ buttons, and close/delete X button. Scales overlays via CSS transform scale scale(currentScale) in untransformed canvas coordinates, rendering via a structured block-line DOM extractor), Size, Zoom, Undo, Save, Close. Saving does not exit the editor; it performs in-place active session updates (with fileSize transmitted to prevent gallery size N/A errors) and displays a green save success toast.
...
- Supports both images and PDF files (with transparent drawing canvas layered on top to preserve original PDF selectable text using `pdf-lib` overlay slice stitching). Separates PDF pages with a subtle 2px visual boundary line (`#cbd5e1`) inside the editor.
