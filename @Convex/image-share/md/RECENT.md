...
- Overhauled display scaling by replacing absolute CSS scale transforms (which caused blurry, pixelated, or poorly anti-aliased font rendering) with browser-independent **direct JavaScript scaling (`fontSize = size * currentScale`)**. Text elements are styled directly with calculated target screen font sizes, forcing the browser to natively rasterize characters perfectly at any zoom scale. Raw canvas sizes are stored in unscaled `data-canvas-size` datasets for perfect canvas exports.
- Integrated a **`ResizeObserver`** on the editor to track active horizontal resizing with safety NaN checks, storing the unscaled canvas width independently (`canvasWidth`). This guarantees text width limits and word wraps scale accurately when zooming.
- Resolved wrapping width mismatches by dynamically dividing the editor's visual bounding width (`getBoundingClientRect().width`) by the current canvas display scale (`currentScale`) to compute the exact untransformed canvas layout width (`maxWidth`). This guarantees word wrapping locations match screen layouts perfectly at any zoom level or user width resizing.
- Added a **dynamic window tracking callback hook** (`window.updateActivePaintTextPosition`) that repositions and rescales the active textbox in-real-time when adjusting zoom sliders or dragging, avoiding cursor drift.
...
- Added **Line Gap customization controls** (`L-` and `L+` buttons) to the floating toolbar, allowing users to customize line heights dynamically between `0.8` and `2.5` times the font size (which updates the editor's inline CSS `line-height` and the canvas layout positioning).
- Fixed a bug where **Ctrl+A (Select All)** inside the contenteditable textbox was hijacked by the global gallery shortcut. Added `isContentEditable` validation to exclude inputs and text annotations from global keyboard listeners.
- Added a **subtle 2px grey separator line** (`#cbd5e1`) between stacked pages in the PDF viewer canvas. This visually splits pages in the editor UI without embedding the lines in the saved output document.
